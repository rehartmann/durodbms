...
RDB_cat_dbtables_insert(RDB_object *tbp, RDB_database *dbp, RDB_exec_context *ecp, RDB_transaction *txp)
{
    ...
    if (RDB_string_to_id(&tbnameobj, RDB_table_name(tbp), ecp) != RDB_OK)
        goto error;
        ...
        RDB_dbroot *dbrootp, RDB_exec_context *ecp, RDB_transaction *txp)
{
    ...
    for (j = 0; j < keyp->strc; j++) {
        ...
        if (RDB_string_to_id(&kaobj, keyp->strv[j], ecp) != RDB_OK) {
        ...
        RDB_exec_context *ecp, RDB_transaction *txp)
{
    ...
    for (i = 0; i < tuptyp->def.tuple.attrc; i++) {
        ...
        if (defaultp != NULL) {
            ...
            if (RDB_string_to_id(&attrnameobj, attrname, ecp) != RDB_OK)
                goto error;
...
delete_rtable(RDB_object *tbp, RDB_exec_context *ecp, RDB_transaction *txp)
{
    ...
    RDB_expression *idexprp = RDB_tablename_id_eq_expr(RDB_table_name(tbp), ecp);
    if (idexprp == NULL) {
        return RDB_ERROR;
    }
...
{
    int ret;
    RDB_expression *exprp = RDB_tablename_id_eq_expr(RDB_table_name(tbp), ecp);
    if (exprp == NULL) {
        return RDB_ERROR;
    }
...
RDB_cat_delete_ptable(const char *tbname, RDB_exec_context *ecp, RDB_transaction *txp)
{
    ...
    idexprp = RDB_tablename_id_eq_expr(tbname, ecp);
    if (idexprp == NULL) {
        ret = RDB_ERROR;
        goto cleanup;
    }
        ...
        const char *name, RDB_exec_context *ecp)
{
    ...
    RDB_add_arg(exp, argp);
    argp = RDB_tablename_id_eq_expr(name, ecp);
    if (argp == NULL) {
        goto error;
    }
    RDB_add_arg(exp, argp);
        ...
        RDB_transaction *txp)
{
    ...
    if (RDB_string_to_id(&tbnameobj, "sys_rtables", ecp) != RDB_OK)
        goto error;
...
string_to_id_expr(const char *str, RDB_exec_context *ecp)
{
    ...
    RDB_init_obj(&obj);
    if (RDB_string_to_id(&obj, str, ecp) != RDB_OK)
        goto error;
        ...
        RDB_transaction *txp)
{
    ...
    idcondp = RDB_tablename_id_eq_expr(RDB_table_name(tbp), ecp);
    if (idcondp == NULL) {
        goto error;
    }
