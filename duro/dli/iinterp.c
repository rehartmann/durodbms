 ...
 */
static int node_to_multi_assign(const RDB_parse_node *nodep,
        ...
        RDB_exec_context *ecp)
{
    ...
    for(;;) {
        ...
        if (firstp->kind == RDB_NODE_TOK) {
            ...
            RDB_init_obj(&srcobjv[(*srcobjcp)++]);
            copyv[(*copycp)].srcp = &srcobjv[(*srcobjcp) - 1];
            if (node_to_copy(&copyv[(*copycp)++], firstp, ecp) != RDB_OK) {
                goto error;
            }
        }
...
static int
exec_explain_assign(const RDB_parse_node *nodep, RDB_exec_context *ecp)
{
    int i;
    RDB_ma_copy copyv[DURO_MAX_LLEN];
    RDB_ma_insert insv[DURO_MAX_LLEN];
    ...
    if (RDB_apply_constraints(insc, insv, updc, updv, delc, delv,
            copyc, copyv, put_constraint_expr, ecp, &txnp->tx) != RDB_OK)
        goto error;
        ...
        switch (firstchildp->val.token) {
            ...
            case TOK_EXPLAIN:
                if (firstchildp->nextp->nextp->nextp == NULL) {
                    ret = exec_explain_assign(firstchildp->nextp->val.children.firstp,
                            ecp);
                } else {
